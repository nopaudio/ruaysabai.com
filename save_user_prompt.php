...
try {
    ...
    if ($user_id) {
        ...
        $limit = 5;
        $escaped_ip = $db->escape($ip_address);
        $where = "(user_id IS NULL OR user_id = 0) AND ip_address = '$escaped_ip' AND DATE(created_at) = CURDATE()";
        $period = 'วันนี้';
    }
    ...
    if ($limit != 999999 && $current_count >= $limit) {
        ...
        $upgrade_message = '';
        if ($user_id && $member_type == 'free') {
            $upgrade_message = ' - สมัครแพ็กเกจเช่าซื้อเพื่อเพิ่มสิทธิ์';
        } elseif (!$user_id) {
            $upgrade_message = ' - สมัครสมาชิกฟรีเพื่อใช้งาน 10 ครั้งต่อวัน';
        }
        
        jsonResponse(false, "คุณใช้สิทธิ์ครบแล้วสำหรับ{$period} ({$status_text}: {$limit} ครั้ง){$upgrade_message}");
    }
    ...
    // ตรวจสอบ ip_address
    $check_ip = $db->query("SHOW COLUMNS FROM user_prompts LIKE 'ip_address'");
    if (!$check_ip || $check_ip->num_rows == 0) {
        $db->query("ALTER TABLE user_prompts ADD COLUMN ip_address VARCHAR(45) AFTER generated_prompt");
        error_log("Added ip_address column");
    }
    
    // ตรวจสอบ user_agent
    $check_agent = $db->query("SHOW COLUMNS FROM user_prompts LIKE 'user_agent'");
    if (!$check_agent || $check_agent->num_rows == 0) {
        $db->query("ALTER TABLE user_prompts ADD COLUMN user_agent TEXT AFTER ip_address");
        error_log("Added user_agent column");
    }
