...
try {
    ...
    $count_result = $db->select("SELECT COUNT(*) as count FROM user_prompts WHERE $where");
    ...
    if ($result) {
        ...
        jsonResponse(true, "บันทึก Prompt สำเร็จ - เหลือสิทธิ์ {$remaining} ครั้ง สำหรับ{$period}");
    } else {
